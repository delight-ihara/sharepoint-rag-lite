...
Non-Commercial License (SharePoint RAG Lite NC-1.0)

詳細は `LICENSE` を参照してください。
商用利用は許可制です。`COMMERCIAL-LICENSE.md` を確認してください。

### 利用ポリシー（要点）

- 参考・検証・学習・非商用利用: OK
- 商用利用（再販、SaaS提供、有償導入、収益活動への組み込み）: 要許諾
- 商用利用の相談窓口: `COMMERCIAL-LICENSE.md`

注: 以前の公開版に適用されていたライセンス条件は、その公開時点の条件に従います。

## 商標・ブランド利用

コードライセンスと、名称・ロゴ・ブランド表現の扱いは別です。
商用再配布やホスティング時に名称・ロゴを利用する場合は、`TRADEMARK.md` を確認してください。
